...
| 字段 | 含义与启用要求 |
| --- | --- |
| `enabled` | 生产保持 `false`，在完成验收的发布 PR 中开启 |
| `provider` | 当前仅支持 `kapa` |
...
当前 demo 不能自动刷新来源，手动上传仍可使用；上传后等待索引完成并记录对应文档 SHA、
语言、上传时间和来源范围。上传成功不等于问答验收通过，也不代表已经建立自动同步。
...
- 手动上传对应的文档 SHA、语言、时间、索引状态，以及真实回答和引用的脱敏证据。
...
具体第三方域名许可、Global/父组来源隔离、真实问答与引用以及部署后的 CSP 验收仍需补齐证据。
这些项阻止真实 AI 上线，不阻止 OINK 升级、AI 关闭的 staging 和本地回归。

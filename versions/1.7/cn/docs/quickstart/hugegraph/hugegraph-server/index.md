...
> 不要把 Gremlin、Cypher 等查询接口直接暴露到公网。生产环境应启用[认证与授权](/versions/1.7/cn/docs/config/config-authentication/)，限制网络访问并保留审计日志；部署建议见[安全指南](/versions/1.7/cn/docs/guides/security/)。
...
3. 可以使用 `-e PASSWORD=xxx` 开启鉴权模式并设置 admin 密码，具体步骤可参考 [Config Authentication](/versions/1.7/cn/docs/config/config-authentication#使用-docker-时开启鉴权模式)。

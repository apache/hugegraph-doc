...
2. 可以使用 `docker run -itd --name=server -p 8080:8080 -e PRELOAD="true" hugegraph/hugegraph` 在启动的时候预加载一个**内置的**样例图。可以通过 `RESTful API` 进行验证。具体步骤可以参考 [5.1.1](/versions/1.3/cn/docs/quickstart/hugegraph/hugegraph-server/#511-%E5%90%AF%E5%8A%A8-server-%E7%9A%84%E6%97%B6%E5%80%99%E5%88%9B%E5%BB%BA%E7%A4%BA%E4%BE%8B%E5%9B%BE) 
3. 可以使用 `-e PASSWORD=123456` 设置是否开启鉴权模式以及 admin 的密码，具体步骤可以参考 [Config Authentication](/versions/1.3/cn/docs/config/config-authentication#使用-docker-时开启鉴权模式) 
...
> 注：`${version}` 为版本号，最新版本号可参考 [Download 页面](/versions/1.3/docs/download/download)，或直接从 Download 页面点击链接下载
...
详细的配置介绍请参考[配置文档](/versions/1.3/docs/config/config-guide)及[配置项介绍](/versions/1.3/docs/config/config-option)。
...
如果想要使用 HugeGraph 鉴权模式，在后面正式启动 Server 之前应按照 [Server 鉴权配置](https://hugegraph-oink.staged.apache.org/versions/1.3/cn/docs/config/config-authentication/) 进行配置。
...
> ```
>
> > 更多其它后端配置可参考[配置项介绍](/versions/1.3/docs/config/config-option)
...
详细的 API 请参考 [RESTful-API](/versions/1.3/docs/clients/restful-api) 文档。
...
请参考[在 IDEA 中配置 Server 开发环境](/versions/1.3/docs/contribution-guidelines/hugegraph-server-idea-setup)

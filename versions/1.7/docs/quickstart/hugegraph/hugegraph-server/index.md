...
> Do not expose Gremlin, Cypher, or other query endpoints directly to the public Internet. In production, enable [authentication and authorization](/versions/1.7/docs/config/config-authentication/), restrict network access, and retain audit logs. See the [Security Guide](/versions/1.7/docs/guides/security/) for deployment guidance.
...
3. You can use `-e PASSWORD=xxx` to enable authentication mode and set the admin password. See [Config Authentication](/versions/1.7/docs/config/config-authentication#use-docker-to-enable-authentication-mode) for details.

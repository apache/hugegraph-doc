...
- [HugeGraph-Server](/versions/1.7/cn/docs/quickstart/hugegraph/hugegraph-server): HugeGraph-Server 是 HugeGraph 项目的核心部分，包含 Core、Backend、API 等子模块；
...
- [HugeGraph-Toolchain](https://github.com/apache/hugegraph-toolchain): (工具链)
  - [HugeGraph-Client](/versions/1.7/cn/docs/quickstart/client/hugegraph-client)：HugeGraph-Client 提供了 RESTful API 的客户端，用于连接 HugeGraph-Server，目前仅实现 Java 版，其他语言用户可自行实现；
  - [HugeGraph-Loader](/versions/1.7/cn/docs/quickstart/toolchain/hugegraph-loader)：HugeGraph-Loader 是基于 HugeGraph-Client 的数据导入工具，将普通文本数据转化为图形的顶点和边并插入图形数据库中；
  - [HugeGraph-Hubble](/versions/1.7/cn/docs/quickstart/toolchain/hugegraph-hubble)：HugeGraph-Hubble 是 HugeGraph 的 Web 
可视化管理平台，一站式可视化分析平台，平台涵盖了从数据建模，到数据快速导入，再到数据的在线、离线分析、以及图的统一管理的全过程；
  - [HugeGraph-Tools](/versions/1.7/cn/docs/quickstart/toolchain/hugegraph-tools)：HugeGraph-Tools 是 HugeGraph 的部署和管理工具，包括管理图、备份/恢复、Gremlin 执行等功能。
- [HugeGraph-Computer](/versions/1.7/cn/docs/quickstart/computing/hugegraph-computer)：HugeGraph-Computer 是分布式图处理系统 (OLAP). 
  它是 [Pregel](https://kowshik.github.io/JPregel/pregel_paper.pdf) 的一个实现。它可以运行在 Kubernetes/Yarn 
  等集群上，支持超大规模图计算。
- [HugeGraph-AI](/versions/1.7/cn/docs/quickstart/hugegraph-ai)：HugeGraph-AI 是 HugeGraph 独立的 AI 
...
- 邮件反馈：[dev@hugegraph.apache.org](mailto:dev@hugegraph.apache.org) ([邮箱订阅方式](https://hugegraph-oink.staged.apache.org/versions/1.7/docs/contribution-guidelines/subscribe/))
- SEC 反馈： [security@hugegraph.apache.org](mailto:security@hugegraph.apache.org) (报告安全相关问题)
- 微信公众号：Apache HugeGraph, 欢迎扫描下方二维码加入我们！

 ![Apache HugeGraph 微信公众号二维码](/versions/1.7/images/docs/community/wechat.png){width="300" height="94"}
